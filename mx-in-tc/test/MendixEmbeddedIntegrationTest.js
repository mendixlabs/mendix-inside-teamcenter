...
jest.mock(
    'https://apps.example.com/dist/embedded-index.js',
    () => ( {
        render: jest.fn()
    } ),
    { virtual: true }
...
describe( 'mendixEmbedded integration', () => {
    ...
    it( 'keeps an initial configuration error while the error panel loads translations', async() => {
        const populateI18nMap =
            localizationService.populateI18nMap.bind( localizationService );
        let pendingTranslations;
        jest
            .spyOn( localizationService, 'populateI18nMap' )
            .mockImplementation( ( ...args ) => {
                if ( args[0].INVALID_URL ) {
                    return new Promise( ( resolve ) => {
                        pendingTranslations = { resolve, args };
                    } );
                }
        ...
        await act( async() =>
            pendingTranslations.resolve(
                await populateI18nMap( ...pendingTranslations.args )
            )
        );
    ...
    it( 'uses XRT configuration and resolves mapped context values', async() => {
        ...
        renderWithCtx(
            <MendixEmbedded
                subPanelContext={{
                    declarativeKeyContext:
                        'https://apps.example.com/?uid={selected.uid}&mode=edit'
                }}
            />,
        ...
        await waitFor( () =>
            expect( renderApp ).toHaveBeenCalledWith(
                ...
                expect.objectContaining( {
                    remoteUrl: 'https://apps.example.com/',
                    parameters: { uid: 'UID-456', mode: 'edit' }
                } )
            )
        );
    } );
    ...
    it( 'reports unsupported object parameters without starting authentication', async() => {
        const view = renderWithCtx(
            <MendixEmbedded config='https://apps.example.com/?item={selected}' />,
            {
                initialState: { selected: { uid: 'UID-456' } }
            }
        );
        await view.findByText(
            'Mendix parameters must resolve to strings, numbers, or booleans.'
        );
        expect( ensureHasValidSession ).not.toHaveBeenCalled();
        expect( renderApp ).not.toHaveBeenCalled();
    } );
    ...
    it( 'shows the localized runtime URL error when session validation cannot be reached', async() => {
        ...
        await view.findByText(
            'Cannot reach the Mendix runtime at the configured URL. Check that the URL is correct and the runtime is running.'
        );
